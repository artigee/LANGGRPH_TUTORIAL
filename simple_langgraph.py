...
setup_environment_variables("LangGraph Basics")
...
runnable_agent = create_openai_functions_agent(LLM, TOOLS, PROMPT)
...
# print("Input Schema:")
# print(runnable_agent.input_schema.model_json_schema())
# # Or get more details about the agent
# print("\nAgent Info:")
# print(runnable_agent.input_schema.model_json_schema()["properties"])  # Shows input properties

#agent_outcome = runnable_agent.invoke(inputs)
#print(agent_outcome)

def agent_node(inputs: AgentState):
    agent_outcome: AgentActionMessageLog = runnable_agent.invoke(inputs)
    return {"agent_outcome": agent_outcome}

tool_executor = ToolExecutor(tools=TOOLS)

def tool_executor_node(inputs: AgentState):
    agent_action = inputs["agent_outcome"]
    output = tool_executor.invoke(agent_action)
    print(f"Excute {agent_action} with output: {output}")
...
#call_weather_app("What is the weather in Tokyo?")
